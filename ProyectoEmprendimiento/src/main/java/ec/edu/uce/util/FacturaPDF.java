...
public class FacturaPDF {
    ...
    public static void generarFactura(Cliente cliente, GasolineraManager gasolinera, TipoGasolina tipoGasolina, float precio, float galones, float total, float iva, float total_iva, String tipoPago, List<TipoAditivo> aditivos) {
        ...
        try {
            ...
            float totalAditivos = agregarDetalleVenta(document, tipoGasolina, galones, total, iva, total_iva, tipoPago, aditivos, gasolinera);
            total += totalAditivos; // Sumar el total de aditivos al total general
    ...
    private static void agregarInformacionCliente(Document document, Cliente cliente, int facturaId) {
    ...
    private static float agregarDetalleVenta(Document document, TipoGasolina tipoGasolina, float galones, float total, float iva, float total_iva, String tipoPago, List<TipoAditivo> aditivos, GasolineraManager gasolinera) {
        ...
        for (TipoAditivo aditivo : aditivos) {
            float precioAditivo = gasolinera.obtenerPrecioAditivo(aditivo);
            table.addCell(new Paragraph(aditivo.getNombre()).setFontSize(8));
            table.addCell(new Paragraph("1").setFontSize(8));
            table.addCell(new Paragraph(String.valueOf(precioAditivo)).setFontSize(8));
            totalAditivos += precioAditivo;
        }
    ...
    private static void agregarPiePagina(Document document, Cliente cliente, GasolineraManager gasolinera, TipoGasolina tipoGasolina, float total) {
